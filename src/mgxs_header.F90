...
!===============================================================================
    ...
    subroutine mgxsiso_from_hdf5(this, xs_id, energy_groups, delayed_groups, &
      ...
      real(8), allocatable        :: temp_3d(:, :, :), temp_beta(:, :)
      real(8)                     :: dmu, mu, norm
      ...
      do t = 1, temps_to_read % size()
        associate(xs => this % xs(t))
          ...
          allocate(xs % delayed_nu_fission(energy_groups, delayed_groups))
          ...
          if (this % fissionable) then
            ...
            if (check_dataset(xsdata_grp, "beta")) then
              ...
              if (ndims == 1) then
                ...
                temp_beta = reshape(temp_arr, (/energy_groups, delayed_groups/))
              ...
              if (ndims == 1) then
                ...
                if (delayed_groups == energy_groups) then
                  call warning("delayed-nu-fission was input as a dimension &
                       ...
                       &delayed_groups). If the intention was to have the &
                       &matrix as (energy_groups, energy_groups), the &
                       &delayed-nu-fission should be separately for each &
                       &delayed group as a (energy_groups, energy_groups, &
                       &delayed_groups) matrix.")
                end if
                ...
                ! Deallocate temporary arrays for delayed_nu_fission matrix
                deallocate(temp_arr)
                deallocate(temp_2d)
                ...
                allocate(temp_arr(delayed_groups * energy_groups * &
                     energy_groups))
                call read_dataset(temp_arr, xsdata_grp, "delayed-nu-fission")
                allocate(temp_3d(energy_groups, energy_groups, delayed_groups))
                temp_3d = reshape(temp_arr, (/energy_groups, energy_groups, &
                     delayed_groups/))
                ...
                do dg = 1, delayed_groups
                  do gin = 1, energy_groups
                    xs % delayed_nu_fission(gin, dg) = sum(temp_3d(:, gin, dg))
                    do gout = 1, energy_groups
                      xs % chi_delayed(gout, gin, dg) = temp_3d(gout, gin, dg)
                    end do
                  end do
                end do
          ...
          if (check_dataset(xsdata_grp, "inverse_velocity")) then
            call read_dataset(xs % inverse_velocity, xsdata_grp, "inverse_velocity")
          else
      ...
      do t = 1, temps_to_read % size()
        associate(xs => this % xs(t))
          ...
          allocate(xs % prompt_nu_fission(energy_groups, this % n_azi, &
               this % n_pol))
          allocate(xs % delayed_nu_fission(energy_groups, this % n_azi, &
               this % n_pol, delayed_groups))
          allocate(xs % chi_prompt(energy_groups, energy_groups, this % n_azi, &
               this % n_pol))
          allocate(xs % chi_delayed(energy_groups, energy_groups, &
          ...
          allocate(xs % kappa_fission(energy_groups, this % n_azi, this % n_pol))
          allocate(xs % decay_rate(this % n_azi, this % n_pol, delayed_groups))
          allocate(xs % inverse_velocity(energy_groups, this % n_azi, this % n_pol))
          ...
          if (this % fissionable) then
            ...
            if (check_dataset(xsdata_grp, "chi")) then
              ...
              allocate(temp_arr(1 * energy_groups * this % n_azi * &
                   this % n_pol))
              call read_dataset(temp_arr, xsdata_grp, "chi")
              ...
              do ipol = 1, this % n_pol
                do iazi = 1, this % n_azi
                  do gout = 1, energy_groups
                    l = l + 1
                    xs % chi_prompt(gout, gin, iazi, ipol) = temp_arr(l)
                  end do

                  ! Normalize chi so its CDF goes to 1
                  xs % chi_prompt(:, gin, iazi, ipol) = &
                       xs % chi_prompt(:, gin, iazi, ipol) / &
                       sum(xs % chi_prompt(:, gin, iazi, ipol))
                end do
              end do
              ...
              if (check_dataset(xsdata_grp, "nu-fission")) then
                ...
                ! Get nu_fission
                call read_dataset(temp_arr, xsdata_grp, "nu-fission")
                xs % prompt_nu_fission(:, :, :) = reshape(temp_arr, &
                  ...
                  do ipol = 1, this % n_pol
                    do iazi = 1, this % n_azi
                      do dg = 1, delayed_groups
                        ...
                        ! Set delayed_nu_fission using delayed neutron fraction
                        xs % delayed_nu_fission(gin, iazi, ipol, dg) = &
              ...
              ! 2) nu_fission (m) provided
              ! ----------------------------------------------------------------
            else if (check_dataset(xsdata_grp, "nu-fission")) then
              ...
              if (check_dataset(xsdata_grp, "chi-prompt")) then
                ...
                call fatal_error("If chi and nu_fission not provided, &
                     &chi_prompt must be provided")
              end if

              ! Set chi_delayed
              if (check_dataset(xsdata_grp, "chi_delayed")) then
                ...
                do ipol = 1, this % n_pol
                  do iazi = 1, this % n_azi
                    do gout = 1, energy_groups
                      do dg = 1, delayed_groups
                        l = l + 1
                        xs % chi_delayed(gout, gin, iazi, ipol, dg) = &
                             temp_arr(l)
                      end do
                    end do

                    ! Normalize chi so its CDF goes to 1
                    xs % chi_delayed(:, gin, iazi, ipol, dg) = &
                         xs % chi_delayed(:, gin, iazi, ipol, dg) / &
                         sum(xs % chi_delayed(:, gin, iazi, ipol, dg))
                  end do
                end do
              ...
              if (check_dataset(xsdata_grp, "delayed-nu-fission")) then
                ...
                call read_dataset(temp_arr, xsdata_grp, "delayed-nu-fission")
                xs % delayed_nu_fission(:, :, :, :) = reshape(temp_arr, &
            ...
            if (check_dataset(xsdata_grp, "fission")) then
              ...
              xs % fission(:, :, :) = reshape(temp_arr, (/energy_groups, &
                   this % n_azi, this % n_pol/))
            ...
            if (check_dataset(xsdata_grp, "kappa-fission")) then
              ...
              xs % kappa_fission(:, :, :) = reshape(temp_arr, (/energy_groups, &
                   this % n_azi, this % n_pol/))
            ...
            if (check_dataset(xsdata_grp, "decay-rate")) then
              ...
              call read_dataset(temp_arr, xsdata_grp, "decay-rate")
              xs % decay_rate(:, :, :) = reshape(temp_arr, (/this % n_azi, &
                   this % n_pol, delayed_groups/))
          ...
          if (check_dataset(xsdata_grp, "absorption")) then
            ...
            xs % absorption = reshape(temp_arr, (/energy_groups, this % n_azi, &
                 this % n_pol/))
          ...
          if (check_dataset(xsdata_grp, "inverse_velocity")) then
            ...
            call read_dataset(temp_arr, xsdata_grp, "inverse_velocity")

            allocate(xs % inverse_velocity(energy_groups, this % n_azi, &
                 this % n_pol))
            xs % inverse_velocity = reshape(temp_arr, (/energy_groups, &
                 this % n_azi, this % n_pol/))
          ...
          if (check_dataset(xsdata_grp, "total")) then
            ...
            xs % total = reshape(temp_arr, (/energy_groups, this % n_azi, &
                                             this % n_pol/))
...
!===============================================================================
    ...
    subroutine mgxsang_combine(this, temps, mat, nuclides, energy_groups, &
      ...
      TEMP_LOOP: do t = 1, temps % size()
        ...
        allocate(this % xs(t) % decay_rate(n_azi, n_pol, delayed_groups))
        this % xs(t) % decay_rate = ZERO
        ...
        allocate(this % xs(t) % delayed_nu_fission(energy_groups, n_azi, &
             n_pol, delayed_groups))
        this % xs(t) % delayed_nu_fission = ZERO
                ...
                do ipol = 1, n_pol
                  do iazi = 1, n_azi
                    do gin = 1, energy_groups
                      norm =  sum(this % xs(t) % chi_prompt(:, gin, iazi, ipol))
                      if (norm > ZERO) then
                        this % xs(t) % chi_delayed(:, gin, iazi, ipol, dg) = &
                             this % xs(t) % chi_delayed(:, gin, iazi, ipol, dg) &
                             / norm
                      end if
                    end do
...
!===============================================================================
    ...
    pure function mgxsang_get_xs(this, xstype, gin, gout, uvw, mu, dg) result(xs)
      ...
      if (present(uvw)) then
        ...
        case('delayed_nu_fission')
          if (present(dg)) then
            xs = this % xs(t) % delayed_nu_fission(gin, iazi, ipol, dg)
          else
        ...
        case('chi_delayed')
          if (present(gout)) then
            if (present(dg)) then
              xs = this % xs(t) % chi_delayed(gout, gin, iazi, ipol, dg)
            else
              xs = this % xs(t) % chi_delayed(gout, gin, iazi, ipol, 1)
            end if
          else
            if (present(dg)) then
              xs = sum(this % xs(t) % chi_delayed(:, gin, iazi, ipol, dg))
            else
              xs = sum(this % xs(t) % chi_delayed(:, gin, iazi, ipol, 1))
            end if
          end if
