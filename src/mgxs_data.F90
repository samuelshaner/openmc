    ...
    MATERIAL_LOOP: do i = 1, n_materials
      ...
      NUCLIDE_LOOP: do j = 1, mat % n_nuclides
        ...
        if (.not. already_read % contains(name)) then
          ...
          ! Read in the data specific to the type we just declared
...
!===============================================================================
  ...
  subroutine create_macro_xs()
    ...
    do i_mat = 1, n_materials
      ...
      ! Allocate data based on the mgxs type
      ...
      ! Creat the macro xs
      call macro_xs(i_mat) % obj % combine(mat, nuclides_MG, num_energy_groups, &
           num_delayed_groups, max_order, scatt_type)
    end do
